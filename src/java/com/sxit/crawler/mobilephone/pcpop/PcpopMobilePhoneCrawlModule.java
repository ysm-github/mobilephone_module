...
public class PcpopMobilePhoneCrawlModule extends CrawlModule{
	private static Logger log = LoggerFactory.getLogger(PcpopMobilePhoneCrawlModule.class);
	
	public static final String BRAND_ID_KEY = "brand.id";
	public static final String BRAND_NAME_KEY = "brand.name";
	
	public final static String RECORD_FILE_NAME = "pcpop_com_mobileparam.data";
	
	private final static String DEFAULT_JOB_NAME = PcpopMobilePhoneCrawlModule.class.getSimpleName();
	...
	public void execute() {
		//数据表配置
		DatatableConfig datatableConfig = initDatatableConfig("pcpop_datatable_config.xml");
		
		//保存需要采集的URL
		Set<String> ids = new ConcurrentSkipListSet<String>();
		
		//手机列表采集器，采集结果为手机的ID编号列表，其结果将放入ids集合变量中。
		PcpopMobilePhoneListCrawlerProcess pcpopMobilePhoneListCrawlerProcess = 
				new PcpopMobilePhoneListCrawlerProcess(USER_AGENT_PROVIDER, this, crawlConfig, ids);
		
		
		//手机详细信息采集，采集的结果直接入库，入库字段依据为datatableConfig中所配置的映射关系
		PcpopMobilePhoneDetailInfoCrawlerProcess pcpopMobilePhoneDetailInfoCrawlerProcess = 
				new PcpopMobilePhoneDetailInfoCrawlerProcess(USER_AGENT_PROVIDER, this, crawlConfig, ids, datatableConfig);
		...
		try {
			...
			pcpopMobilePhoneListCrawlerProcess.process(param);
		...
		try {
			log.info("开始采集手机信息");
			startTime = System.currentTimeMillis();
			pcpopMobilePhoneDetailInfoCrawlerProcess.process(param);
			endTime = System.currentTimeMillis();
